...
__align(4) uint8_t Rx_Buff[ETH_RX_BUF_SIZE*ETH_RXBUFNB]; 	
...
u8 LAN8720_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOD, ENABLE);//使能GPIO时钟 RMII接口
	...
	/*网络引脚设置 RMII接口
	  ...
	  ETH_RMII_RXD0 --------------------> PC4
	  ETH_RMII_RXD1 --------------------> PC5//???
	  ETH_RMII_TX_EN -------------------> PG11//ERR	PB11
	  ETH_RMII_TXD0 --------------------> PG13//ERR	PB12
	  ETH_RMII_TXD1 --------------------> PG14
	  ETH_RESET-------------------------> PD3*/
	...
	GPIO_PinAFConfig(GPIOG, GPIO_PinSource11, GPIO_AF_ETH); //引脚复用到网络接口上
