...
int main(void)
{
    ...
    CREATE_OS_TASK(system_monitor_task);   // 创建初始化任务
    CREATE_OS_TASK(remote_lcd_task);       // 创建遥控器显示屏任务
    CREATE_OS_TASK(read_remote_ctrl_task); // 创建手柄键盘扫描任务
...
void read_remote_ctrl_task(void *p) // 手柄键盘扫描任务
{
    OS_ERR err;
    p = p;
    OSTimeDly_ms(500);
...
void remote_lcd_task(void *p) // esp8266发送任务
{
    OS_ERR err;
    p = p;
    OSTimeDly_ms(100);
...
void transmit_task(void *p)
{
    ...
    while (1)
    {
        ...
        USART3_DMA_Tx(); // 注碌鹁嵅禄禄釓采徛板发藣

        // 赂酶蓮掳宸⒙的?-3位蕠sucker_slide拢卢4-7位蕠sucker_lift
        OSTimeDly_ms(1);
    }
}
