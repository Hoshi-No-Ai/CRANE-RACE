...
/*定义主任务的堆栈长度*/
#define navigation_task_SIZE 512
#define read_remote_ctrl_task_SIZE 512
#define motor_control_task_SIZE 512
#define system_monitor_task_SIZE 256
#define debug_task_SIZE 256
#define remote_lcd_task_SIZE 512
#define temp_test_task_SIZE 256
#define action_task_SIZE 512
#define transmit_task_SIZE 512

/*定义主任务的优先级*/
#define read_remote_ctrl_task_PRIO 4
#define navigation_task_PRIO 6
#define motor_control_task_PRIO 8
#define remote_lcd_task_PRIO 10
#define action_task_PRIO 7
#define debug_task_PRIO 11
#define system_monitor_task_PRIO 9
#define transmit_task_PRIO 12
...
#define DECLARE_HITCRT_OS_TASK()            \
    DECLARE_OS_TASK(read_remote_ctrl_task); \
    DECLARE_OS_TASK(navigation_task);       \
    DECLARE_OS_TASK(motor_control_task);    \
    DECLARE_OS_TASK(system_monitor_task);   \
    DECLARE_OS_TASK(debug_task);            \
    DECLARE_OS_TASK(remote_lcd_task);       \
...
#define CREATE_OS_TASK(NAME)                                                                     \
    OSTaskCreate((OS_TCB *)&NAME##TCB, (CPU_CHAR *)#NAME, (OS_TASK_PTR)NAME, (void *)0,          \
                 (OS_PRIO)NAME##_PRIO, (CPU_STK *)&NAME##Stk[0], (CPU_STK_SIZE)NAME##_SIZE / 10, \
                 (CPU_STK_SIZE)NAME##_SIZE, (OS_MSG_QTY)0, (OS_TICK)0, (void *)0,                \
...
#define CREATE_OS_TMR(NAME, DLY, PERIOD, OPT)                                                        \
    OSTmrCreate((OS_TMR *)&NAME##TMR, (CPU_CHAR *)#NAME, (OS_TICK)DLY, (OS_TICK)PERIOD, (OS_OPT)OPT, \
                (OS_TMR_CALLBACK_PTR)(OS_TMR_CALLBACK_PTR)&NAME##_CallBack, (void *)0,               \
                (OS_ERR *)&NAME##ERROR)
...
入口参数：NAME，定时器名称
---------------------------------------------------------------------------------------------------*/
#define DECLARE_OS_FLAG(NAME) OS_FLAG_GRP NAME##FLAG

// 声明创建的事件标志组
#define DECLARE_RC_OS_FLAG() DECLARE_OS_FLAG(CAN)
...
#define CREATE_OS_FLAG(NAME) \
    OSFlagCreate((OS_FLAG_GRP *)&NAME##FLAG, (CPU_CHAR *)"NAME##_FLAG", (OS_FLAGS)0x00, (OS_ERR *)&err)
...
#define OSFlagConsumePend(NAME)                                        \
    OSFlagPend((OS_FLAG_GRP *)&NAME##FLAG, (OS_FLAGS)0x0F, (OS_TICK)0, \
               (OS_OPT)OS_OPT_PEND_FLAG_SET_ANY + OS_OPT_PEND_FLAG_CONSUME, (CPU_TS *)0, (OS_ERR *)&err)
...
#define OSFlagNonConsumePend(NAME)                                                                       \
    OSFlagPend((OS_FLAG_GRP *)&NAME##FLAG, (OS_FLAGS)0x0F, (OS_TICK)0, (OS_OPT)OS_OPT_PEND_FLAG_SET_ANY, \
               (CPU_TS *)0, (OS_ERR *)&err)
...
入口参数：NAME，ORDER，标志位
---------------------------------------------------------------------------------------------------*/
#define OSFlagQuickPost(NAME, ORDER)                                                                        \
    do                                                                                                      \
    {                                                                                                       \
