...
*******************************************************************************/
extern u16 count_thousand;
u8 key_scan_1,key_scan_2,key_scan_3;
u16 KeyStat;
u16 count_ms_thousand;
extern u32 sys_minor;
extern int m;
void SysTick_Handler(void)
{
  ...
  count_thousand++;
	...
	if(count_thousand > 1000)
	{

		count_ms_thousand++;
		
		if(count_ms_thousand>1000)
		{
			count_ms_thousand = 0;
			sys_minor = 0;
			m = 0;
...
s16 CRR3;
s16 CRR4;
CanRxMsg RxMessage_VELVE;
extern u16 TempValveStat;
void USB_LP_CAN1_RX0_IRQHandler(void)
{
	CAN_Receive(CAN1, CAN_FIFO0, &RxMessage_VELVE);		//接收数据
	LED_ROLLING;
	switch( RxMessage_VELVE.StdId )	 //判断ID
		{
			/*气动控制*/
			case 0x40:
				memcpy(&CRR1, RxMessage_VELVE.Data , 2);
				memcpy(&CRR2, RxMessage_VELVE.Data + 2, 2);
				memcpy(&CRR3, RxMessage_VELVE.Data + 4 ,2);
				memcpy(&CRR4, RxMessage_VELVE.Data + 6, 2);
				break;
			case 0x41:
			break;
			case CANRXVALVEID:
				TempValveStat = ((RxMessage_VELVE.Data[1]<<8) + RxMessage_VELVE.Data[0]);
			break;
			default:
			break;			
		}
...
void TIM4_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM4, TIM_IT_Update) != RESET) //检查指定的TIM中断发生与否:TIM 中断源 
		...
		TIM_SetCompare1(TIM3, CRR1);
		TIM_SetCompare2(TIM3, CRR2);
		TIM_SetCompare3(TIM3, 1150 + CRR3);
		TIM_SetCompare4(TIM3, 1150 + CRR4);
		
		ValveCtrl();
		sys_minor++;
		
		SendKeyStat();
		ucKeyScanFlag = 1 ;		//按键检测标志位置为1
//			
//			if(usKeyStat == 0x02)
//			{
//				key_scan_1 = 1;
...
//				key_scan_3 = 0;
//			}
//			SendUSHORT16ByCan(CANTEKID_PLUS,key_scan_3+key_scan_1*2+key_scan_2*4);			//经消抖处理后检测到不同值，通过CAN发送
//		
}
